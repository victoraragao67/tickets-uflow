    ...
    <div className="flex h-screen flex-col bg-background">
      <header className="flex items-center justify-between px-8 py-5">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-0.5">{clients.length} clientes</p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-8 scrollbar-thin">
        {showAdd && (
          <div className="rounded-2xl bg-card card-shadow p-5 mb-5">
            <h3 className="text-sm font-semibold mb-4 text-foreground">Novo Cliente</h3>
            <div className="grid grid-cols-2 gap-3">
              <Input placeholder="Nome do Cliente *" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.name} onChange={(e) => setNewClient({ ...newClient, name: e.target.value })} />
              <Input placeholder="Empresa" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.company} onChange={(e) => setNewClient({ ...newClient, company: e.target.value })} />
              <Input placeholder="Segmento" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.segment} onChange={(e) => setNewClient({ ...newClient, segment: e.target.value })} />
              <Input placeholder="Nome do Contato" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.contactName} onChange={(e) => setNewClient({ ...newClient, contactName: e.target.value })} />
              <Input placeholder="E-mail" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.email} onChange={(e) => setNewClient({ ...newClient, email: e.target.value })} />
              <Input placeholder="Telefone" className="h-10 rounded-xl border-border bg-background text-sm focus-visible:ring-ring" value={newClient.phone} onChange={(e) => setNewClient({ ...newClient, phone: e.target.value })} />
            </div>
            <div className="flex gap-2 mt-4">
              <button onClick={handleAdd} className="h-10 px-5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity shadow-sm">Criar</button>
              <button onClick={() => setShowAdd(false)} className="h-10 px-5 rounded-xl border border-primary text-primary bg-card text-sm font-medium hover:bg-accent transition-colors">Cancelar</button>
            </div>
          </div>
        )}
